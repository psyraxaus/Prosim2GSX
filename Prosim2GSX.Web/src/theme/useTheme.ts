...
export function useTheme(themeName: string | undefined | null): void {
  const { get } = useApi();
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        const theme = await get<ThemeFile>(
          `/appsettings/theme/${encodeURIComponent(themeName)}`
        );
        if (!cancelled && theme && theme.colors) applyTheme(theme.colors);
      } catch {
