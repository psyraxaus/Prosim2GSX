...
export function NotificationBanner() {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        const snap = await get<NotificationsSnapshotDto>("/notifications");
        if (!cancelled) {
          dispatch({
            type: "set",
  ...
  useEffect(() => {
    if (!current) return;
    if (current.severity !== "info") return;
    if (dismissTimers.current.has(current.id)) return;

    const handle = window.setTimeout(() => {
      // Fire-and-forget — the WS broadcast will mark it dismissed,
      // which makes the banner pick the next-most-recent entry.
      post(`/notifications/${current.id}/dismiss`).catch(() => { /* ignore */ });
    }, 10_000);
  ...
  return (
    <div className={`${styles.banner} ${styles[`sev_${current.severity}`]}`}>
      <span className={styles.severity}>{current.severity.toUpperCase()}</span>
