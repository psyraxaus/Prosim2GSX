...
namespace Prosim2GSX.UI.NotifyIcon
{
    public partial class NotifyIconModelExt(ISimApp simApp) : NotifyIconViewModel(simApp)
    {
        ...
        {
            try { (SimApp as Prosim2GSX).AppService.ResetRequested = AppResetRequest.App; } catch { }
        }

        [RelayCommand]
        public virtual void RestartAppGsx()
        {
            try { (SimApp as Prosim2GSX).AppService.ResetRequested = AppResetRequest.AppGsx; } catch { }
        }
    }
}
