...
namespace Prosim2GSX
{
    public static class Tools
    {
        public static string GetMsfsWindowTitle()
        {
            ...
            try
            {
                Process proc = Process.GetProcessesByName(Prosim2GSX.Instance.Config.BinaryMsfs2024)?.FirstOrDefault();
                if (!string.IsNullOrWhiteSpace(proc?.MainWindowTitle))
                    result = proc.MainWindowTitle;
                else
                    result = $"{Prosim2GSX.Instance.Config.Msfs2024WindowTitle}{proc?.MainModule?.FileVersionInfo?.FileVersion?.Replace(',', '.')}";
            }
            catch (Exception ex)
            {
